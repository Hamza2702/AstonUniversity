...
public class WordGuessMain {
	...
	protected void askUserToGuess() {
		...
		if (candidates.isEmpty()) {
			...
			if (candidates.contains(guess)) {
				...
				System.out.println("The word was " + candidates);
			}
		}
	}
	...
	protected boolean processLine(String pattern) {
		// TODO needs to be implemented
		if (pattern.isEmpty()) {
			return true ;// Buildup of candidates is done and program should switch to asking the user to make a guess
		}
		...
		if (cmd == '+') {
			candidates = wordList.searchWords(firstLetter, substring);
		} else if (cmd == '-') {
			candidates.removeAll(wordList.searchWords(firstLetter, substring));
		} else if (cmd == '*') {
			candidates.retainAll(wordList.searchWords(firstLetter, substring)); // Updating candidates to the intersection of its old value and the results
		} else {
